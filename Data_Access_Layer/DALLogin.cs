...
namespace Data_Access_Layer
{
    public class DALLogin
    {
        ...
        public async Task<string> UpdateUserAsync(UpdateUserRequest user)
        {
            try
            {
                var existingUser = await _cIDbContext.User.Where(x => !x.IsDeleted && x.Id == user.Id).FirstOrDefaultAsync();
                var existingUserDetail = await _cIDbContext.UserDetail.Where(x => !x.IsDeleted && x.UserId == user.Id).FirstOrDefaultAsync();
        ...
        {
            return await _cIDbContext.User.Where(x => !x.IsDeleted && x.Id == id).FirstOrDefaultAsync();
        }
